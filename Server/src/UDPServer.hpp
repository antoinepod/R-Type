...
private:
    void start_receive()
    {
        ...
        if (myMap.empty() || search == myMap.end())
            gameObject = Network::populatePlayerObject(gameObject, myMap.size() + 1, 100, 420);
        //if (!remote_endpoint_.address().to_string().compare("127.0.0.1"))
        myMap[remote_endpoint_.address().to_string() + std::to_string(remote_endpoint_.port())] = (myMap.size());
